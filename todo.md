...
- [x] U-010 rollout: applied the approved restricted role, least-privilege grants, direct-client privilege revocations, and explicit RLS policy migrations on Supabase without changing `DATABASE_URL`; platform-owned default privileges remain a separately governed follow-up.
- [x] U-010 rollout: verified role attributes, grants, RLS posture, policies, and the existing production valuation path; the role is non-bypass and all eight governed tables have RLS, while the unchanged production connection remains `postgres` with `BYPASSRLS`.
- [x] U-010 rollout: corrected the migration's owner-qualified default-privilege statement after Supabase rejected it; a platform-privileged future default-privilege hardening task is recorded rather than falsely claiming that restriction.
- [x] U-010 rollout: removed the unexecutable default-privilege statement after the Supabase executor rejected it even for the current `postgres` session; revoked direct `anon` and `authenticated` grants from the eight governed existing tables, and recorded future default-privilege hardening as a platform-privileged follow-up.
- [x] U-010 rollout blocker: the Supabase executor has `CREATEROLE` but cannot execute `ALTER ROLE` against the platform-owned role hierarchy; three role/policy migration attempts failed atomically, followed by a successful guarded-create application. RLS remains enabled on all eight tables, while the production connection continues to use `postgres` with `BYPASSRLS`.
- [x] U-010 rollout: owner approved and PR #62 merged the removal of the failing `ALTER ROLE miayaar_app` statement; the guarded `CREATE ROLE` path retains every explicitly restricted attribute, while `DATABASE_URL` remains unchanged.
- [x] U-010 governance exception: recorded the owner-approved temporary `LOGIN + PASSWORD NULL` posture for `miayaar_app`; it does not meet the final `NOLOGIN` target, contains no stored password, and must be corrected only through a separately approved database-administrator phase.
- [ ] U-010 deferred follow-up: correct `miayaar_app` to `NOLOGIN` through a separately approved database-administrator phase; do not change `DATABASE_URL`, credentials, role membership, or policies without a new owner decision.
- [x] U-010 NOLOGIN administrative package: documented the current executor-permission blocker and prepared only the administrator SQL, pre-checks, post-checks, application non-impact verification, and single-attribute change record; no SQL was executed and no code, migration, connection setting, credential, membership, grant, or RLS policy was changed.
- [x] U-010 closure decision: recorded the owner decision that U-010 is complete as a software-delivered RLS hardening path; deferred only the `NOLOGIN` administrative correction to a future database-administrator phase and returned to the governed build roadmap without starting its next implementation stage.
- [x] Notion Project Knowledge Base gate: extract the linked Notion specifications and owner-provided reconciliation content; compare them with the current repository, methodology v1.2, frozen contracts, and governance decisions before any new build implementation.
- [x] Requirements Reconciliation Matrix: record each Notion requirement against current implementation, missing work, conflict, and status; classify unsupported items as `UNRESOLVED` or `CONFLICT / GOVERNANCE REVIEW` without creating product rules.
- [x] Methodology §4–§5 implementation: extract the approved property-type-to-method applicability, per-type weights, and required fields directly from `docs/VALUATION-METHODOLOGY.md` §§4–5 and reconcile them with the frozen v1.2 configuration.
- [x] Methodology §4–§5 implementation: apply the extracted method applicability and approved weights to server-side method selection without changing any unrecorded coefficient, threshold, fallback, or valuation formula.
